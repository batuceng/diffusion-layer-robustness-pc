...
def get_logits(data, model, shift, scale, device):
    if isinstance(data, list):
        logits = []
        for i in data:
            logit = model(i.permute(0, 2, 1).to(device))
            logits.append(logit)
...
# Load defense model
if args.defense == "sor":
    defense_model = SORDefense().to(device)
else:
    defense_model = SRSDefense().to(device)
defense_model.eval()
...
savedir = os.path.join(*[args.save_path, args.defense, args.attack, args.model]) if args.save_path is not None else None
