...
class PointMLP(nn.Module):
    ...
    def forward(self, x):
        xyz = x.permute(0, 2, 1)
        batch_size, _, _ = x.size()
        x = self.embedding(x)  # (B,D,N), D=64, N=1024

        for i in range(self.stages):
            # (B,D,N), i:0 D=128, N=512;   i:1 D=256, N=256;      i:2 D=512, N=128;      i:3 D=1024, N=64;
...
def download_pointmlp(root):
    ...
    SUB_DIR = os.path.join(DATA_DIR, 'pointmlp_weights')
