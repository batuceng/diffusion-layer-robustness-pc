...
class Pct(nn.Module):
    ...
    def forward_denoised(self, x, denoiser=None):
        ...
        layer_data = []
        # Layer 0 
        x = denoiser(x, layer=0)
        layer_data.append(x.clone().detach().requires_grad_(True))
        ...
        new_xyz, new_feature = sample_and_group(npoint=512, radius=0.15, nsample=32, xyz=xyz, points=x)         
        feature_0 = self.gather_local_0(new_feature)
        feature_0 = denoiser(feature_0, layer=1)
        layer_data.append(feature_0.clone().detach().requires_grad_(True))
        ...
        # PCT Layers (3 & 4)
        x, layers = self.pt_last.forward_denoised(feature_1, denoiser=denoiser)
...
def download_pct(root):
    ...
    SUB_DIR = os.path.join(DATA_DIR, 'pct_weights')
    if not os.path.exists(DATA_DIR):
        os.mkdir(DATA_DIR)
    if not os.path.exists(SUB_DIR):
        os.mkdir(SUB_DIR)
    if not os.path.exists(os.path.join(SUB_DIR, 'model.t7')):
        www = 'https://github.com/Strawberry-Eat-Mango/PCT_Pytorch/raw/main/checkpoints/best/models/model.t7'
...
class Point_Transformer_Last(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.sa1(x)#torch.Size([16, 256, 256])
        x2 = self.sa2(x1)#torch.Size([16, 256, 256])
    ...
    def forward_denoised(self, x, denoiser):
        ...
        x1 = self.sa1(x)#torch.Size([16, 256, 256])
        x2 = self.sa2(x1)#torch.Size([16, 256, 256])
        x2 = denoiser(x2, layer=3)
        layers.append(x2.clone().detach().requires_grad_(True))

        #Layer 4
        x3 = self.sa3(x2)#torch.Size([16, 256, 256])
        x4 = self.sa4(x3)#torch.Size([16, 256, 256])
        x4 = denoiser(x4, layer=4)
        layers.append(x4.clone().detach().requires_grad_(True))
