...
class DGCNN(nn.Module):
    def __init__(self, output_channels=40, mode="main_train"):
    ...
    def forward(self, x, 
        ...
        batch_size = x.size(0)
        original = x.clone()
        x = denoiser[0].denoise_layer(x, t, layer_name="original") # Denoising input
        x = get_graph_feature(x, k=self.k)
        x = self.conv1(x)
        x1 = x.max(dim=-1, keepdim=False)[0]
        x1 = denoiser[1].denoise_layer(x1, t, layer_name="x1") # Denosing Layer1

        x = get_graph_feature(x1, k=self.k)
        x = self.conv2(x)
        x2 = x.max(dim=-1, keepdim=False)[0]
        x2 = denoiser[2].denoise_layer(x2, t, layer_name="x2") # Denosing Layer2

        x = get_graph_feature(x2, k=self.k)
        x = self.conv3(x)
        x3 = x.max(dim=-1, keepdim=False)[0]
        x3 = denoiser[3].denoise_layer(x3, t, layer_name="x3") # Denosing Layer3

        x = get_graph_feature(x3, k=self.k)
        x = self.conv4(x)
        x4 = x.max(dim=-1, keepdim=False)[0]
        x4 = denoiser[4].denoise_layer(x4, t, layer_name="x4") # Denosing Layer4
